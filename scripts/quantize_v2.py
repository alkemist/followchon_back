...
from utils import save_quantized_model, prepare_quantization_dataset, export2openvino


def quantize_basic(ov_model, pt_modelpath, args, ignored_scope=None, datapath='./', yaml_datapath='../data.yaml',
                   get_model_graph=False):
...
def quantize_AC(ov_model, validation_metric, pt_modelpath, args, ignored_scope=None, datapath='./',
    ...
    * The resulted model can provide smaller performance improvement than the Basic 8-bit quantization \
    ...
    quantization_dataset, validator, data_loader = prepare_quantization_dataset(pt_modelpath, args, datapath,
                                                                                yaml_datapath)
...
def main_basic(pt_modelpath, args, get_model_graph=False, datapath='../datasets/images', yaml_datapath='../data.yaml'):
    ...
    quantized_basic_model = quantize_basic(ov_model=ov_model, pt_modelpath=pt_modelpath, args=args,
                                           ignored_scope=ignored_scope, datapath=datapath,
                                           get_model_graph=get_model_graph, yaml_datapath=yaml_datapath)
...
def main_AC(validation_metric, pt_modelpath, args, datapath, yaml_datapath='../data.yaml'):
    ...
    quantized_AC_model = quantize_AC(ov_model=ov_model, datapath=datapath, validation_metric=validation_metric,
                                     pt_modelpath=pt_modelpath, yaml_datapath=yaml_datapath, args=args)
    return quantized_AC_model


def main():
    pt_modelpath = '../guinea-pig-chons-v11.pt'
    yaml_datapath = 'data.yaml'
    datapath = '../data'  # path to images folder
    settings.update({"datasets_dir": "../"})
    args = get_cfg(cfg=DEFAULT_CFG)
    # RUN QUANTIZATION
    main_basic(pt_modelpath, args, datapath=datapath, yaml_datapath=yaml_datapath,
               get_model_graph=True)  # or main_AC(validation_metric, pt_modelpath, args)
